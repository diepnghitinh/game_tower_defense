...
@implementation CBusiStation2

-(id)init
{
    if (self=[super init])
    {
        animation = [CGlobalAnimation Station2];
        info = [[CInfoStation2 alloc] init];
...
-(void)Exec:(CGlobalCamera2D*)camera coordinate:(int)index
{
    ...
    body = [CCSprite spriteWithSpriteFrameName:[objectManager.FirstObject objectForKey: @"1"]];
...
-(void)DrawStatus
{
    ...
    if (status > 8)
    {
        ...
        if (skill != nil)
        {
            ...
            [[SimpleAudioEngine sharedEngine] playEffect:@"station-2-1.mp3"];
            return;
        }
    }
