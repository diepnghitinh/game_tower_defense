...
@implementation CBusiStation4

-(id)init
{
    if (self=[super init])
    {
        animation = [CGlobalAnimation Station4];
        info = [[CInfoStation4 alloc] init];
...
-(void)Exec:(CGlobalCamera2D*)camera coordinate:(int)index
{
    ...
    body = [CCSprite spriteWithSpriteFrameName:[objectManager.FirstObject objectForKey: @"6"]];
...
-(void)attackSkill2End
{
    
    if ([self Distance] <= info.attack && enemyIndentify != nil && enemyIndentify.deading == NO)
    {
        [enemyIndentify Hits: [[[info.randomSkill objectAtIndex: 1] objectAtIndex:1] damage]*info.level];
        [self BloodEffect];
    }
    else
    {
...
-(void)Skill2Draw
{
    ...
    {
        case sattack1:
            action = [CCSequence actions: [CCAnimate actionWithAnimation:[CCAnimation animationWithFrames: [animation objectForKey:@"s2-attack2"] delay:0.1f]], 
                      [CCCallFunc actionWithTarget:self selector:@selector(attackSkill2End)], nil];
            action= [CCRepeat actionWithAction:action times:[self CalDelay:[animation objectForKey:@"s2-attack2"]]];
            [self runAction:action];
            break;
        case sattack2:
            action = [CCSequence actions: [CCAnimate actionWithAnimation:[CCAnimation animationWithFrames: [animation objectForKey:@"s2-attack2"] delay:0.1f]], 
        ...
        case sattack4:
            ...
            {
                action = [CCSequence actions: [CCAnimate actionWithAnimation:[CCAnimation animationWithFrames: [animation objectForKey:@"s2-attack1"] delay:0.1f]], 
                          [CCCallFunc actionWithTarget:self selector:@selector(attackSkill2End)], nil];
                action= [CCRepeat actionWithAction:action times:[self CalDelay:[animation objectForKey:@"s2-attack1"]]];
                body.flipX = YES;
            }
            [self runAction:action];
            break;
    ...
    if (status > 8)
    {
        ...
        if (skill != nil)
        {
            ...
            if (skill.name == @"skill1")
            {
                ...
                [[SimpleAudioEngine sharedEngine] playEffect:@"male-5-0.mp3"];
            }
