...
@implementation CBusiStation5

-(id)init
{
    if (self=[super init])
    {
        animation = [CGlobalAnimation Station5];
        info = [[CInfoStation5 alloc] init];
        soundAttack = @"male-5-0.mp3";
    }
    return self;
}
...
-(void)Exec:(CGlobalCamera2D*)camera coordinate:(int)index
{
    ...
    body = [CCSprite spriteWithSpriteFrameName:[objectManager.FirstObject objectForKey: @"7"]];
...
-(void)attackSkill1End
{
    ...
    {
        if (enemyIndentify.statusEffect == 1)
        {
            id action = [CGlobalActionSlow actionWithDuration:[[[info.randomSkill objectAtIndex: 0] objectAtIndex:1] slow] target:enemyIndentify];
            [enemyIndentify runAction: action];
        }
...
-(void)DrawStatus
{
    ...
    if (status > 8)
    {
        ...
        if (skill != nil)
        {
            ...
            [[SimpleAudioEngine sharedEngine] playEffect:@"male-5-1.mp3"];
            return;
        }
    }
