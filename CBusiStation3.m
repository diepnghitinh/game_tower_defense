...
@implementation CBusiStation3

-(id)init
{
    if (self=[super init])
    {
        animation = [CGlobalAnimation Station3];
        info = [[CInfoStation3 alloc] init];
        soundAttack = @"female-1.mp3";
    }
    return self;
}
...
-(void)Exec:(CGlobalCamera2D*)camera coordinate:(int)index
{
    ...
    body = [CCSprite spriteWithSpriteFrameName:[objectManager.FirstObject objectForKey: @"2"]];
...
-(void)DrawStatus
{
    ...
    if (status > 8)
    {
        ...
        if (skill != nil)
        {
            ...
            [[SimpleAudioEngine sharedEngine] playEffect:@"female-2.mp3"];
            return;
        }
    }
